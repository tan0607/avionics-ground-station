...
void initPyro(bool verbose) {
  ...
  Serial.println("[HAND TEST PYRO] Gate LOW, channel SAFE");

  if (pyroFired) {
    Serial.println("[HAND TEST PYRO] *** LATCH SAYS ALREADY FIRED ***");
    Serial.println("[HAND TEST PYRO] A fire request was latched; deployment is NOT confirmed.");
    Serial.println("[HAND TEST PYRO] Arming is blocked. Power cycle to clear.");
  }

#if !ARM_SWITCH_ENABLED
  Serial.println("[HAND TEST PYRO] NOTE - arm switch sense disabled in Config.h");
#endif

#if PYRO_CONT_ENABLED
  readContinuity();
  Serial.print("[HAND TEST PYRO] Continuity ");
  Serial.print(contOK ? "OK" : "OPEN");
  Serial.print(" (");
  Serial.print(contVolts, 2);
  Serial.println(" V)");
#else
  Serial.println("[HAND TEST PYRO] NOTE - continuity sense disabled in Config.h");
#endif
}
...
bool armPyro() {
  if (!pulseTimerReady || pyroFiring) {
    Serial.println("[HAND TEST PYRO] ARM REFUSED - pulse timer unavailable or output active");
    return false;
  }

  if (pyroFired) {
    Serial.println("[HAND TEST PYRO] ARM REFUSED - already fired this flight");
    return false;
  }

  if (!armSwitchClosed()) {
    Serial.println("[HAND TEST PYRO] ARM REFUSED - arm switch is open");
    return false;
  }
...
#if PYRO_CONT_ENABLED
  if (!contOK) {
    Serial.println("[HAND TEST PYRO] ARM REFUSED - no continuity, check the match");
    return false;
  }
#endif
  ...
  Serial.println("[HAND TEST PYRO] *** ARMED ***");
  return true;
}
...
void disarmPyro() {
  ...
  Serial.println("[HAND TEST PYRO] DISARMED - gate LOW");
}
...
bool firePyro(const char* reason) {
  if (!pyroArmed) {
    Serial.println("[HAND TEST PYRO] FIRE REFUSED - not armed");
    return false;
  }

  if (pyroFired) {
    Serial.println("[HAND TEST PYRO] FIRE REFUSED - already fired");
    return false;
  }

  if (!preparePulse()) {
    Serial.println("[HAND TEST PYRO] FIRE REFUSED - pulse timer unavailable or output active");
    return false;
  }
  ...
  if (!startPulse()) {
    ...
    Serial.println("[HAND TEST PYRO] FIRE FAILED - timer start; latch remains set, gate LOW");
    return false;
  }
  fireCount++;
...
bool testFirePyro() {
  if (pyroArmed) {
    Serial.println("[HAND TEST PYRO] TEST REFUSED - disarm first");
    return false;
  }
...
void servicePyro() {
  if (pyroFiring) {
    if (pulseCutoffDone || millis() - fireStartTime >= FIRE_DURATION) {
      ...
      Serial.print("[HAND TEST PYRO] Pulse cutoff serviced at elapsed ");
