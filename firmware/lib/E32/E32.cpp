...
void E32::begin(int rxPin, int txPin, uint32_t uartBaud) {
  pinMode(_m0, OUTPUT);
  pinMode(_m1, OUTPUT);
...
bool E32::waitAux(uint32_t timeoutMs) {
  uint32_t start = millis();
  while (digitalRead(_aux) == LOW) {
    if (millis() - start >= timeoutMs) return false;  // radio busy -> caller must not write
    delay(1);
  }
  return true;
}

void E32::setMode(E32Mode mode) {
...
bool E32::writeFrame(const uint8_t* data, size_t len, uint32_t timeoutMs) {
  if (!waitAux(timeoutMs)) return false;  // <-- the anti-lock-up guarantee: never blind-write
  _s.write(data, len);
  return true;
}

bool E32::configure(uint8_t addrHigh, uint8_t addrLow, uint8_t channel,
                    uint8_t sped, uint8_t option) {
  setMode(E32_CONFIG);            // M0=1 M1=1: parameters can only be written in mode 3
  while (_s.available()) _s.read();  // flush any stale bytes

  // C0 = "set parameters and save through power-down" (use C2 for volatile).
  ...
  while (got < sizeof(resp) && millis() - start < 1000) {
    if (_s.available()) resp[got++] = _s.read();
  }

  setMode(E32_NORMAL);            // back to transparent TX/RX for flight
