...
# Finder, or run ./start.command from a terminal.
#
...
#
#   ./start.command                 live: find the ground station on USB
#   ./start.command --demo          no hardware: simulated flight, looping
#   ./start.command --replay flights/2026-08-19T05-54-40Z
#   ./start.command --dev           Vite hot reload in front of a live backend
#   ./start.command --serial /dev/cu.usbserial-0001 --no-browser
#
...
if ! "$PY" -c 'import fastapi, uvicorn, serial' >/dev/null 2>&1; then
  step "Installing backend dependencies"
  "$PY" -m pip install --quiet --disable-pip-version-check fastapi uvicorn pyserial \
